                    ...
                    <div class="col-xs-12 col-sm-6 col-md-6 col-md-offset-3 profile1" style="padding-bottom:40px;">
                        <div class="thinborder-ontop">
                            ...
                            <form id="userregisterationform" method="POST" action="{{ route('register') }}">
                                ...
                                <div class="form-group row">
                                    <label for="register_as" class="col-md-4 col-form-label text-md-right">Register
                                        As</label>
                                    <div class="col-md-6">
                                        <select id="register_as" name="register_as" class="form-control" required="">
                                            <option>Select Register As :</option>
                                            @foreach (\App\Enums\UserTypeEnum::cases() as $utype)
                                            @if ($utype->name == 'ADMIN')
                                                @continue
                                            @endif
                                                <option value="{{ $utype->value }}">{{ Str::replace('_', ' ', $utype->name) }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
